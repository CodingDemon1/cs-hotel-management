const baseURL = "https://hotelmanagementcodsoft1.onrender.com";
...
function getData() {
	...
	fetch(`${baseURL}/booking/${logedInUserID}`, {
		headers: {
...
function appendData(data) {
	if (data.length == 0) {
		...
		data.forEach((element) => {
			...
			if (status.textContent == "Pending") status.style.color = "orange";
			if (status.textContent == "Paid") status.style.color = "green";
				...
				btn2.addEventListener("click", function () {
					Swal.fire({
						...
						if (result.isConfirmed) {
							...
							fetch(`${baseURL}/booking/${bookingId}/cancel`, {
								method: "PUT",
								headers: {
